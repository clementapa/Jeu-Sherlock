...
int main(int argc, char ** argv)
{
	...
	while (!quit)
		...
		if (SDL_PollEvent(&event))
		{
			//printf("un event\n");
				...
				case  SDL_MOUSEBUTTONDOWN:
					...
					/* On met un si le click est dans la case correspondant*/
					if(mx>=640 && my<=128*3){
					mx=(mx-640)/128;
					my=my/128;
					M[my][mx]=1-M[my][mx];
		...
		SDL_Rect rectBitmap1 = { 300, 260, 500, 330 };
        	SDL_RenderCopy(renderer, textureBitmap1, NULL, &rectBitmap1);
		...
		SDL_RenderDrawLine(renderer, 1024-384,128*3,1024-384,0);
		SDL_RenderDrawLine(renderer, 1024-384+128,128*3,1024-384+128,0);
		SDL_RenderDrawLine(renderer, 1024-384+128+128,128*3,1024-384+128+128,0);

		for( i=0;i<3;i++){
			for ( j=0;j<3;j++){
				if(M[i][j]==1){
					SDL_RenderDrawLine(renderer, 640+j*128,i*128,640+j*128+128,i*128+128);
				}
			}
		}


		SDL_RenderDrawLine(renderer, 1024-384,128,1024,128);
		SDL_RenderDrawLine(renderer, 1024-384,128*2,1024,128*2);
		SDL_RenderDrawLine(renderer, 1024-384,128*3,1024,128*3);
